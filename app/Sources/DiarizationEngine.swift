...
actor DiarizationEngine {
    ...
    private func ingest(_ chunk: AudioChunk) {
        ...
        while pending.count - readIndex >= win {
            ...
            if (vad.isInSpeech || wasInSpeech) && vad.lastWindowWasSpeech {
                // Segment open and this window is actual speech (skip
                // hangover/silence windows — their near-floor log-mel frames
                // would swamp the speaker embedding identically for every
                // voice): keep features.
    ...
    /// `lexicon`: custom vocabulary threaded through to the Transcriber's
    /// contextual strings (AppState → here → Transcriber).
    func finish(audioFileURL: URL,
                locale: Locale = Locale(identifier: "en_US"),
                lexicon: [String] = []) async throws -> DiarizedTranscript {
        ...
        return DiarizedTranscript(utterances: utterances,
                                  ...
                                  speakerCentroids: clusterer.centroidsByRelabeledIndex())
    }
}
