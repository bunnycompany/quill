...
public actor AudioRecorderEngine {
    ...
    // -- stop signalling (integration: UI requests stop; pipeline awaits it) --
    private var stopRequested = false
    private var stopContinuation: CheckedContinuation<Void, Never>?

    public init() {}

    /// Called from the UI to end the session; wakes `waitForStop()`.
    ...
    /// Suspends until `requestStop()` is called, then tears down and
    /// returns the finished recording.
    public func waitForStop() async throws -> RecordingResult {
        if !stopRequested {
            await withCheckedContinuation { (cont: CheckedContinuation<Void, Never>) in
                if stopRequested { cont.resume() } else { stopContinuation = cont }
            }
        }
        stopRequested = false
