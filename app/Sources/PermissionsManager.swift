...
final class PermissionsManager: ObservableObject {
    ...
    /// Mic-only recording is Quill's primary path, so only the microphone
    /// gates the record button. Screen recording is needed only when the
    /// user enables the system-audio toggle.
    var allGranted: Bool { micStatus == .granted }
