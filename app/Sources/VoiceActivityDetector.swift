...
final class VoiceActivityDetector {
    ...
    /// True iff the most recent window fed to `process` was classified as
    /// speech (lets the engine skip silence/hangover windows when
    /// accumulating embedding features).
    private(set) var lastWindowWasSpeech = false
    ...
    func process(rms: Float, zcr: Float, windowStart: TimeInterval) -> VADSegment? {
        ...
        lastWindowWasSpeech = isSpeechWindow
